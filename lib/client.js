/**
 * dsh-whale-musume client plugin: injects the Umika desktop mascot into the DSH Web UI (pure DOM injection).
 *
 * No built-in package files are modified or product text read. Voice requests stay on the local host.
 ...
 * scripts/apply-theme.mjs --mascot-settings v29 (reads whale-moe:* localStorage and
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /* Custom self-name: pairs with "What should I call you"; empty falls back to the default "Umika". */
    function MascotSelfNameRow({ compact }) {
      return jsxs("label", { style: compact ? { alignItems: "center", display: "flex", gap: "12px", justifyContent: "space-between", padding: "2px 0" } : MASCOT_ROW_STYLE, children: [jsx("span", { children: "Her name for herself" }), jsx("input", {
        type: "text",
        defaultValue: MascotValue("selfName", "Umika"),
        maxLength: 12,
        placeholder: "Umika",
        style: { flex: 1, maxWidth: "150px", minWidth: 0 },
        onChange: (event) => {
    ...
    /* Opens the presenter's own "Manage Dialog" modal (built in dsh-whale-moe.js,
       exposed as window.__dshWhaleMoeOpenDialogManager since this settings panel
       and the presenter share the same page/window). */
    function MascotDialogRow() {
      return jsxs("div", { style: { display: "flex", flexDirection: "column", gap: "6px", padding: "4px 0 6px", width: "100%" }, children: [
        jsx("span", { style: { color: "var(--dsw-alias-label-secondary)", fontSize: "12px", lineHeight: "16px" }, children: "View, mute, or rewrite any of her fixed lines." }),
        jsx("button", { type: "button", onClick: () => { if (typeof window.__dshWhaleMoeOpenDialogManager === "function") window.__dshWhaleMoeOpenDialogManager(); }, children: "Manage Dialog Lines…" })
      ] });
    }
    function MascotVoiceRow() {
      const [language, setLanguage] = React.useState(MascotValue("voiceLanguage", "ja"));
      const [voice, setVoice] = React.useState(MascotValue("kokoroVoice:" + language, language === "ja" ? "jf_tebukuro" : "af_sarah"));
      const voices = language === "ja" ? ["jf_tebukuro", "jf_alpha", "jf_gongitsune", "jf_nezumi"] : ["af_sarah", "af_bella", "af_nicole", "af_sky"];
      React.useEffect(() => {
        const sync = () => {
          const next = MascotValue("voiceLanguage", "ja");
          setLanguage(next);
          setVoice(MascotValue("kokoroVoice:" + next, next === "ja" ? "jf_tebukuro" : "af_sarah"));
        };
      ...
      const [status, setStatus] = React.useState("Checking voice setup…");
      const refresh = () => fetch("/api/dsh-whale-musume/voice/status")
        .then((response) => response.json())
        .then((data) => setStatus(data.installed ? (data.error ? "Voice error: " + data.error : "Kokoro ready · cache " + Math.round(data.cacheBytes / 1048576) + " MB") : "Setup needed: npm run setup:voice"))
        .catch(() => setStatus("Voice sidecar unavailable"));
      React.useEffect(() => { refresh(); }, []);
      return jsxs("div", { style: { display: "flex", flexDirection: "column", gap: "6px", padding: "4px 0 8px" }, children: [
        jsxs("label", { style: MASCOT_ROW_STYLE, children: [jsx("span", { children: "Dialogue language" }), jsxs("select", {
          value: language,
          onChange: (event) => {
          ...
          children: [jsx("option", { value: "ja", children: "Japanese" }), jsx("option", { value: "en", children: "English" })]
        })] }),
        jsxs("label", { style: MASCOT_ROW_STYLE, children: [jsx("span", { children: "Kokoro voice" }), jsx("select", { value: voices.includes(voice) ? voice : voices[0], onChange: (event) => {
          const value = event.target.value;
          setVoice(value);
        ...
        }, children: voices.map((name) => jsx("option", { value: name, children: name }, name)) })] }),
        jsx("span", { style: { color: "var(--dsw-alias-label-secondary)", fontSize: "11px" }, children: status }),
        jsx("button", { type: "button", onClick: () => fetch("/api/dsh-whale-musume/voice/clear", { method: "POST" }).then(refresh).catch(() => setStatus("Could not clear voice cache")), children: "Clear voice cache" })
      ] });
    }
    ...
    function MascotPrefRows({ ctx }) {
      return jsxs("div", { style: { display: "flex", flexDirection: "column", width: "100%" }, children: [
        ...
        jsxs(MascotAccordion, { title: "Companion behaviour", icon: "🎛️", summary: "How Umika appears and talks", defaultOpen: true, children: [jsx(MascotSwitchGrid, { ctx }), jsx(MascotVoiceRow, {})] }),
        ...
        jsx(MascotAccordion, { title: "Growth journal", icon: "📖", summary: "Memories with " + MascotValue("selfName", "Umika"), defaultOpen: false, children: [jsx(MascotJournalRow, {})] }),
        jsx(MascotAccordion, { title: "Dialog lines", icon: "🗨️", summary: "Mute or edit what she says", defaultOpen: false, children: [jsx(MascotDialogRow, {})] }),
